  ...
  const handleSubmit = async (event) => {
    ...
    try {
      try {
        ...
        message.success("Accout created successfully");
        navigate("/");
      } catch (error) {
        message.error(error?.response?.data ?? "Please enter correct inputs");
      }
    } catch (err) {
      console.log(err);
      message.error(err.response.data ?? "there wa/s an error");
    }
  };

  return (
    <div className="bg-[#c8cceb] h-screen flex items-center ">
      <div
        style={{ margin: "0 auto" }}
        className="bg-white w-[80%] h-[90%] rounded-lg flex"
      >
        <div className="max-w-96  h-full flex flex-col justify-center items-center ">
          <div className="pr-24">
            <h1 className="bold pb-1 md:pb-4  text-[#101540] font-bold text-2xl">
              Sign Up
            </h1>
            <p className="text-sm font-bold">Create your account to login</p>
          </div>
          <form action="" className=" p-8 pt-0 md:pt-8" onSubmit={handleSubmit}>
            <div className="flex flex-col text-sm p-4"></div>
            <div className="flex flex-col text-sm p-4 pt-0 md:pt-4 font-semibold">
            ...
            <div className="pt-2.5 flex p-4  justify-between ">
              ...
              <a href="" className="text-[14px] text-[#101540] font-semibold ">
                privacy policie and fee
              </a>
            </div>
